...
let s:pref_low = get(g:, 'vim_dadbod_prefer_lowercase', 0)

function! s:prefer_lower(word)
    return s:pref_low ? tolower(a:word) : a:word
endfunction
...
function! vim_dadbod_completion#omni(findstart, base)
  ...
  let table_scope = toupper(s:get_scope(buf, table_scope_match, 'table'))
  call vim_dadbod_completion#utils#msg('table_scope_match ' . table_scope_match . ', table_scope ' . table_scope)
  let buffer_table_scope = toupper(s:get_scope(buf, buf.table, 'table'))
  let schema_scope = toupper(s:get_scope(buf, table_scope_match, 'schema'))
  call vim_dadbod_completion#utils#msg('schema_scope ' . schema_scope)
  ...
  if empty(table_scope) && empty(schema_scope)
    ...
    if !is_trigger_char
      ...
      call map(reserved_words, {i,word -> {'word': s:prefer_lower(word), 'abbr': s:prefer_lower(word), 'menu': s:mark, 'info': 'SQL reserved word', 'kind': 'R' }})
    endif
  ...
  if !empty(schema_scope)
    if !empty(cache_db.objects[schema_scope])    	
		let tables = copy(cache_db.objects[schema_scope])
		if should_filter
			call filter(tables, 'v:val.name =~? s:filter_rgx.a:base')
		endif
"		call vim_dadbod_completion#utils#msg(printf('tables %s', tables))
		call map(tables, function('s:map_item', ['object', '', '']))
	else
...
function! s:map_item(type, info_val, kind, index, item) abort
  let word = a:type ==? 'string' ? a:item : a:type ==? 'list' ? a:item[1] : a:item.name
  let abbr = a:type ==? 'string' ? a:item : a:type ==? 'list' ? split(a:item[1], '(')[0] : a:item.name
  let info = a:type ==? 'string' ? a:info_val : a:type ==? 'list' ? printf(a:item[3]) : printf(a:item.info)
  let kind = a:type ==? 'object' ? toupper(a:item.type) : a:type ==? 'list' ? printf(a:item[2]) : a:kind
  return {
        \ 'word': s:quote(word),
        \ 'abbr': abbr,
        \ 'menu': s:mark,
...
function! s:parse_schemas(db, schemas) abort
  ...
  for schema in data
    ...
    if !has_key(s:cache[a:db].objects, schema[0])
		let s:cache[a:db].objects[schema[0]] = []
    endif
    call add(s:cache[a:db].objects[schema[0]], object)
  endfor
"	call vim_dadbod_completion#utils#msg(printf('s:cache[a:db].objects %s', s:cache[a:db].objects))
...
"  call vim_dadbod_completion#utils#msg(printf('Result %s', a:result))
...
"  call vim_dadbod_completion#utils#msg(printf('Cols after %s', s:cache[a:db].columns_by_table))
endfunction
...
function! s:get_scope(buffer, table_scope, type) abort
  ...
  if a:type ==? 'schema'
    if has_key(cache_db.schemas, a:table_scope) || has_key(cache_db.schemas, toupper(a:table_scope))
	    return a:table_scope
    endif
    return ''
  endif

  if has_key(cache_db.columns_by_table, a:table_scope) || has_key(cache_db.columns_by_table, toupper(a:table_scope))
  call vim_dadbod_completion#utils#msg(printf('Table scope %s', a:table_scope))
    return a:table_scope
  endif
